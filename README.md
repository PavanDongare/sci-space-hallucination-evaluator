...
- [Inputs Ingested by the Evaluator (SciSpace Run Artifacts)](#inputs-ingested-by-the-evaluator-scispace-run-artifacts)
...
## Inputs Ingested by the Evaluator (SciSpace Run Artifacts)

The evaluation tool, `run_evaluator.py`, does not generate research reports; it audits and fact-checks reports generated by external pipelines. It accepts a directory containing the following output artifacts from a SciSpace execution:

1. **User Query (`user_query.txt`):** The original research prompt provided by the user, representing the target intent.
2. **Search Queries Log (`search_queries.txt` / `search_queries.json`):** The raw log of database-specific queries generated during the search phase. Different channels use different query styles (e.g. natural language for semantic search, keyword strings for full-text search, Boolean query syntax for Google Scholar, and MeSH terms for PubMed). Our evaluator reads these queries and evaluates their coverage semantically.
3. **Intermediate Outline Report (`intermediate_report.md`):** The synthesized outline report generated from the retrieved literature.
4. **Final Synthesized Report (`final_report.md`):** The final report containing inline bibliography citations (e.g. `[1]`, `[2]`) and a `## References` section.
5. **Cited Literature Metadata (CSV database):** Spreadsheet caches containing the titles, authors, and abstract texts of papers retrieved from the literature search. These act as the primary local grounding source for evaluation.
...
Our product is the **SciSpace Hallucination Evaluator** CLI (`run_evaluator.py`). It is a command-line tool that ingests the raw SciSpace run artifacts (inputs), orchestrates NLI-based judge checks, audits citation grounding against local caches and API fallbacks, and outputs structured scorecards and detailed logs.

### Evaluator Architecture & Data Flow

```text
  [ SciSpace Run Files (Inputs) ]
     ├── user_query.txt
     ├── search_queries.txt / .json
     ├── intermediate_report.md
     ├── final_report.md
     └── Cited Papers Metadata (CSV)
               │
               ▼
   ┌────────────────────────────────────────────────────────┐
   ...
   │               (run_evaluator.py)                       │
   │                                                        │
   │  1. Parse & Classify Inputs                            │
   │  2. Stage 1: Intent Extraction & Search Query Coverage │
   │  3. Stage 2a: Section-level Directional Alignment      │
   │  4. Stage 2b: CSV Data Extraction NLI Verification     │
   │  5. Stage 2c: Report Synthesis Faithfulness NLI Check  │
   │  6. Stage 3: Claim Extraction & Citation Grounding     │
   │     ├─ Fetch Reference Abstracts (CSV/API Fallbacks)   │
   │     └─ NLI Fact-Checking Judgments                     │
   └────────────────────────────────────────────────────────┘
               │
               ▼
  [ Evaluation Summaries (Outputs) ]
     ├── scorecard.md (Detailed Evidence Tables)
     └── detailed_log.json (Structured Evals Logs)
```
